...
app.get('/games', async (req, res) => {
    try {
        ...
        const gamesWithDesc = result.rows.map(game => ({
            ...
            name: game.game_name,
            image: game.image_filename ? `http://localhost:3000/images/games/${game.image_filename}` : null,
...
app.post('/reservations', async (req, res) => {
    try {
        const { user_id, setup_id, device_type, device_id, reservation_date, start_time, duration_hours, total_price } = req.body;
...
app.post('/devices/available', async (req, res) => {
    try {
        const { device_type, reservation_date, start_time, duration_hours } = req.body;
